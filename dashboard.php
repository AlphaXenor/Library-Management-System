...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    switch ($action) {
        case 'add_book':
            if (!$isPrivileged) {
                $errors[] = 'Only ' . ucfirst($user['role']) . ' and managers can add books.';
                break;
            }
            ...
            try {
                ...
                $stmt->execute();
                $messages[] = ucfirst($user['role']) . ' added book successfully.';
                $stmt->close();
            } catch (mysqli_sql_exception $e) {
                $errorMsg = $e->getMessage();
                // Format common database errors
                if (strpos($errorMsg, 'Duplicate entry') !== false) {
                    if (strpos($errorMsg, 'ISBN') !== false) {
                        $errors[] = ucfirst($user['role']) . ': Book with this ISBN already exists. Please use a different ISBN.';
                    } else {
                        $errors[] = ucfirst($user['role']) . ': ' . $errorMsg;
                    }
                } else {
                    $errors[] = ucfirst($user['role']) . ': Failed to add book. ' . $errorMsg;
                }
                if (isset($stmt)) {
                    $stmt->close();
                }
        ...
        case 'update_book':
            if (!$isPrivileged) {
                $errors[] = 'Only ' . ucfirst($user['role']) . ' and managers can update books.';
                break;
            }
            ...
            try {
                ...
                $stmt->execute();
                $messages[] = ucfirst($user['role']) . ' updated book successfully.';
                $stmt->close();
            } catch (mysqli_sql_exception $e) {
                $errorMsg = $e->getMessage();
                $errors[] = ucfirst($user['role']) . ': Failed to update book. ' . $errorMsg;
                if (isset($stmt)) {
                    $stmt->close();
                }
            }
        ...
        case 'delete_book':
            if (!$isPrivileged) {
                $errors[] = 'Only ' . ucfirst($user['role']) . ' and managers can delete books.';
                break;
            }
            ...
            try {
                ...
                if ($stmt->affected_rows > 0) {
                    $messages[] = ucfirst($user['role']) . ' deleted book successfully.';
                } else {
                    $errors[] = ucfirst($user['role']) . ': No book found with the provided ID.';
                }
                $stmt->close();
            } catch (mysqli_sql_exception $e) {
                $errorMsg = $e->getMessage();
                $errors[] = ucfirst($user['role']) . ': Failed to delete book. ' . $errorMsg;
                if (isset($stmt)) {
                    $stmt->close();
                }
            }
        ...
        case 'fines_period':
            if (!$isPrivileged) {
                $errors[] = 'Only ' . ucfirst($user['role']) . ' and managers can view fines by period.';
                break;
            }
        ...
        case 'add_member':
            if (!$isPrivileged) {
                $errors[] = 'Only ' . ucfirst($user['role']) . ' and managers can add members.';
                break;
            }
            ...
            try {
                ...
                if (strpos($errorMsg, 'Duplicate entry') !== false) {
                    if (strpos($errorMsg, 'Email') !== false) {
                        $errors[] = ucfirst($user['role']) . ': Member with this email already exists. Please use a different email.';
                    } else {
                        $errors[] = ucfirst($user['role']) . ': ' . $errorMsg;
                    }
                } else {
                    $errors[] = ucfirst($user['role']) . ': Failed to add member. ' . $errorMsg;
                }
            } catch (Exception $e) {
                $mysqli->rollback();
                $errors[] = ucfirst($user['role']) . ': Failed to add member. ' . $e->getMessage();
            }
            break;
        ...
        case 'add_staff':
            ...
            try {
                ...
                $mysqli->commit();
                $messages[] = ucfirst($user['role']) . " added staff '$name' successfully as $role (User ID: $userId).";
            } catch (mysqli_sql_exception $e) {
                $mysqli->rollback();
                $errorMsg = $e->getMessage();
                // Format common database errors
                if (strpos($errorMsg, 'Duplicate entry') !== false) {
                    if (strpos($errorMsg, 'Email') !== false) {
                        $errors[] = ucfirst($user['role']) . ': Staff with this email already exists. Please use a different email.';
                    } else {
                        $errors[] = ucfirst($user['role']) . ': ' . $errorMsg;
                    }
                } else {
                    $errors[] = ucfirst($user['role']) . ': Failed to add staff. ' . $errorMsg;
                }
            } catch (Exception $e) {
                $mysqli->rollback();
        ...
        case 'borrow_book':
            ...
            $borrowMemberId = (int) ($_POST['borrow_member_id'] ?? 0);
            ...
            // Member can only borrow for themselves
            if ($user['role'] === 'member') {
                if (!isset($user['member_id'])) {
                    $errors[] = 'Member ID not found in session.';
                    break;
                }
                $borrowMemberId = (int) $user['member_id'];
            } else {
                // Manager/staff can borrow for any member
                if ($borrowMemberId <= 0) {
                    $errors[] = 'Please provide a valid member ID.';
                    break;
                }
            ...
            $stmt = $mysqli->prepare('SELECT Copies_Available, Title FROM BOOKS WHERE Book_ID = ?');
            $stmt->bind_param('i', $bookId);
            $stmt->execute();
            $bookResult = $stmt->get_result()->fetch_assoc();
            $stmt->close();
            
            if (!$bookResult) {
                $errors[] = 'Book not found with the provided ID.';
                break;
            }
            
            if ($bookResult['Copies_Available'] <= 0) {
                $errors[] = 'Book "' . $bookResult['Title'] . '" is not available for borrowing.';
                break;
            }
            
            // Check if member exists
            $stmt = $mysqli->prepare('SELECT User_ID FROM MEMBER WHERE Member_ID = ?');
            $stmt->bind_param('i', $borrowMemberId);
            $stmt->execute();
            $memberResult = $stmt->get_result()->fetch_assoc();
            $stmt->close();
            ...
            // Check if member already has any active borrowings (one book at a time rule)
            $stmt = $mysqli->prepare(
            ...
            $stmt->bind_param('i', $borrowMemberId);
            $stmt->execute();
            $activeBorrowResult = $stmt->get_result()->fetch_assoc();
            $stmt->close();
            
            if ($activeBorrowResult['active_borrows'] > 0) {
                $errors[] = 'Member #' . $borrowMemberId . ' already has an active borrowed book. Please return it before borrowing another book.';
                break;
            }
            
            // Check if member already borrowed this specific book and hasn't returned it
            $stmt = $mysqli->prepare(
                "SELECT COUNT(*) AS book_borrowed 
                 FROM BORROWING 
                 WHERE Member_ID = ? AND Book_ID = ? AND (Status = 'Borrowed' OR Status = 'Overdue')"
            );
            $stmt->bind_param('ii', $borrowMemberId, $bookId);
            $stmt->execute();
            $bookBorrowResult = $stmt->get_result()->fetch_assoc();
            $stmt->close();
            
            if ($bookBorrowResult['book_borrowed'] > 0) {
                $errors[] = 'Member #' . $borrowMemberId . ' already has this book borrowed. Cannot borrow the same book twice.';
                break;
            }
            
            // Get staff_id (if user is staff/manager, use their staff_id, otherwise use default staff ID 1)
            $staffId = isset($user['staff_id']) ? (int) $user['staff_id'] : 1;
            ...
            try {
                ...
                $stmt = $mysqli->prepare(
                    'INSERT INTO BORROWING (Member_ID, Book_ID, Staff_ID, Borrow_Date, Due_Date, Status) 
                     VALUES (?, ?, ?, ?, ?, ?)'
                );
                $stmt->bind_param('iiisss', $borrowMemberId, $bookId, $staffId, $borrowDate, $dueDate, $status);
                $stmt->execute();
                $borrowId = $mysqli->insert_id;
                $stmt->close();
                
                // Update book copies available
                $stmt = $mysqli->prepare('UPDATE BOOKS SET Copies_Available = Copies_Available - 1 WHERE Book_ID = ?');
                $stmt->bind_param('i', $bookId);
                $stmt->execute();
                $stmt->close();
                
                $mysqli->commit();
                $messages[] = 'Book "' . $bookResult['Title'] . '" borrowed successfully for Member #' . $borrowMemberId . ' (Borrow ID: ' . $borrowId . ')';
            } catch (mysqli_sql_exception $e) {
                $mysqli->rollback();
                $errorMsg = $e->getMessage();
                $errors[] = 'Failed to borrow book. ' . $errorMsg;
                if (isset($stmt)) {
                    $stmt->close();
                }
            }
    ...
    <main class="content-wrapper">
        ...
        <section class="card">
            ...
            <form method="POST" class="form-inline">
                ...
                <label>Book ID<input type="number" name="borrow_book_id" required></label>
                <?php if ($isPrivileged): ?>
                    <label>Member ID<input type="number" name="borrow_member_id" required></label>
                <?php endif; ?>
                <button class="primary-btn" type="submit">Borrow Book</button>
            </form>
            <?php if ($user['role'] === 'member'): ?>
                <p class="caption">You can borrow books for yourself (Member ID: <?= (int) $user['member_id']; ?>)</p>
            <?php else: ?>
                <p class="caption">Enter Book ID and Member ID to process borrowing (14-day loan period)</p>
            <?php endif; ?>
        </section>
